...
class MinerNotFull:
   ...
   def try_transform_miner(self, world, transform):
      ...
      if self != new_entity:
         self.clear_pending_action(world)
   ...
   def create_miner_not_full_action(self, world, i_store):
      def action(current_ticks):
         ...
         new_entity = self
         if found:
            new_entity = self.try_transform_miner(world, actions.try_transform_miner_not_full)
   ...
   def clear_pending_action(self, world):
      for action in self.get_pending_actions():
         world.unschedule_action(action)
      self.clear_pending_actions()
...
class MinerFull:
   ...
   def create_miner_full_action(self, world, i_store):
      def action(current_ticks):
         ...
         new_entity = self
         if found:
            new_entity = self.try_transform_miner(world, actions.try_transform_miner_full)
         new_entity.schedule_action(world,
            new_entity.create_miner_action(world, i_store),
            current_ticks + new_entity.get_rate())
         return tiles
